...
typedef struct TCPContext {
    ...
    int64_t pre_addrinfo_intptr;

    int addrinfo_one_by_one;
    int addrinfo_timeout;

    struct addrinfo *pre_addrinfo;
    char *pre_addrinfo_host;
...
static const AVOption options[] = {
    ...
    { "addrinfo",   "pre lookup addrinfo",                                     OFFSET(pre_addrinfo_intptr),   AV_OPT_TYPE_INT64, { .i64 = 0 }, INT64_MIN, INT64_MAX, .flags = D },
    { "addrinfo_host",   "pre lookup addrinfo host name",                      OFFSET(pre_addrinfo_host),   AV_OPT_TYPE_STRING, { .str = NULL }, 0, 0, .flags = D },
...
static int tcp_open(URLContext *h, const char *uri, int flags)
{
    ...
    int needToFreeAddrInfo = 1;
    if(s->pre_addrinfo_intptr) {
        s->pre_addrinfo = (struct addrinfo *)(intptr_t)s->pre_addrinfo_intptr;
    } else {
        s->pre_addrinfo = NULL;
    }
    ...
    av_application_on_tcp_will_getaddrinfo(s->app_ctx);

    if(s->pre_addrinfo && strcmp(s->pre_addrinfo_host, hostname) == 0) {
        printf("[preload] Using pre loaded addr info\n");
        ai = s->pre_addrinfo;    
        needToFreeAddrInfo = 0;
    }
    else {
#ifdef HAVE_PTHREADS
 ...
 restart:
...
#endif
    av_application_on_tcp_did_getaddrinfo(s->app_ctx, 0, 0);
    ...
    if(needToFreeAddrInfo) freeaddrinfo(ai);
