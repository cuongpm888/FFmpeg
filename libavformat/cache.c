...
static int cache_open(URLContext *h, const char *arg, int flags, AVDictionary **options)
{
    ...
    int len = strlen(c->temp_dir) + 12;
    char prefix[len+1];
    snprintf(prefix, len, "%s/ffcache", c->temp_dir);
    c->fd = avpriv_tempfile(prefix, &buffername, 0, h);
#else
    c->fd = avpriv_tempfile("ffache", &buffername, 0, h);
#endif
    if (c->fd < 0){
        av_log(h, AV_LOG_ERROR, "Failed to create tempfile\n");
        return c->fd;
    }
    printf("[Debug] cached file name is: %s\n", buffername);
    unlink(buffername);
    av_freep(&buffername);
...
static int add_entry(URLContext *h, const unsigned char *buf, int size)
{
    ...
    if (!entry ||
        ...
        printf("[Debug] add entry at logical_pos %lld physical_pos %lld size %d \n", entry->logical_pos, entry->physical_pos, size);
    } else {
        entry->size += ret;
        printf("[Debug] append entry at logical_pos %lld physical_pos %lld size %d \n", entry->logical_pos, entry->physical_pos, size);
    }
...
static const AVOption options[] = {
    ...
    { "temp_dir", "for ios where temp dir located", OFFSET(temp_dir), AV_OPT_TYPE_STRING, { .str = "" }, .flags = D },
    {NULL},
};
