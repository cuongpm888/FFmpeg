...
int av_application_on_tcp_will_getaddrinfo(AVApplicationContext *h) {
    if (h && h->func_on_app_event) {
        AVAppTcpIOControl control = {0};
        return h->func_on_app_event(h, AVAPP_CTRL_WILL_GET_ADDR_INFO, (void *)&control, sizeof(AVAppTcpIOControl));
    }
    return 0;
}

int av_application_on_tcp_did_getaddrinfo(AVApplicationContext *h, int error, int fd) {
    if (h && h->func_on_app_event) {
        AVAppTcpIOControl control = {0};
        return h->func_on_app_event(h, AVAPP_CTRL_DID_GET_ADDR_INFO, (void *)&control, sizeof(AVAppTcpIOControl));
    }
    return 0;
}
